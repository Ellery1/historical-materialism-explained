...
CHAPTERS = [
    ("前言", [
        os.path.join(PROJECT_DIR, "00-前言.md"),
    ]),
    ("第一章 · 思想溯源与理论形成", [
        os.path.join(PROJECT_DIR, "01-思想溯源与理论形成.md"),
    ]),
    ("第二章 · 核心原理上：社会存在与社会意识", [
        os.path.join(PROJECT_DIR, "02-核心原理上-社会存在与社会意识.md"),
    ]),
    ("第三章 · 核心原理下：社会发展规律与动力", [
        os.path.join(PROJECT_DIR, "03-核心原理下-社会发展规律与动力.md"),
    ]),
    ("第四章 · 历史主体论", [
        os.path.join(PROJECT_DIR, "04-历史主体论.md"),
    ]),
    ("第五章 · 社会形态更替理论", [
        os.path.join(PROJECT_DIR, "05-社会形态更替理论.md"),
    ]),
    ("第六章 · 实践验证与历史检验", [
        os.path.join(PROJECT_DIR, "06-实践验证与历史检验.md"),
    ]),
    ("第七章 · 常见误解与澄清", [
        os.path.join(PROJECT_DIR, "07-常见误解与澄清.md"),
    ]),
    ("第八章 · 当代价值与现实意义", [
        os.path.join(PROJECT_DIR, "08-当代价值与现实意义.md"),
    ]),
    ("结语", [
        os.path.join(PROJECT_DIR, "结语.md"),
    ]),
    ("参考文献", [
        os.path.join(PROJECT_DIR, "参考文献目录.md"),
    ]),
]
...
def build_content(chapter_data):
    """构建正文 HTML。"""
    content_parts = []
...
def generate_html():
    ...
    for part_title, files_in_part in CHAPTERS:
        chapters_info = []
        part_heading_ids = []

        for filepath in files_in_part:
    ...
    # 构建正文
    content_html = build_content(chapter_data)
    ...
    <style>
        ...
        * {{
        ...
        #back-to-top {{
            ...
            width: 44px;
            height: 44px;
            background: linear-gradient(135deg, #8b6914 0%, #5a3c1e 100%);
            color: #fff;
            border: none;
            border-radius: 50%;
            font-size: 20px;
...
<body>
    ...
    <div class="layout">
        ...
        <div id="main">
            ...
            <footer class="page-footer">
                ...
                <p>生成日期：{gen_date}</p>
                <p>本书基于马克思、恩格斯、列宁等经典著作，系统阐述唯物史观的基本原理</p>
            </footer>
        </div>
    ...
    <script>
        ...
        // ============================================================
        (function() {{
            var STORAGE_KEY = 'hm_book_scroll';
            var saveTimer = null;
